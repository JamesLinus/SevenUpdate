...
namespace SharpBits.Base
{
    ...
    internal interface IBackgroundCopyJob2
    {
        ...
        ///   A string that contains the name of the file on the server. For information on specifying the remote name,
        ///   see the RemoteName member and Remarks section of the <see
        ///    cref="BGFileInfo" /> structure.
        /// </param>
        void AddFile(
        ...
        /// </summary>
        /// <param name="val">
        ///   Contains data that you can use to calculate the percentage of the job that is complete. For more
        ///   information, see <see
        ///    cref="BGJobProgress" />.
        /// </param>
        ...
        /// </summary>
        /// <param name="val">
        ///   An <c>IBackgroundCopyCallback</c> interface pointer. To remove the current callback interface pointer, set
        ...
        /// </summary>
        /// <param name="seconds">
        ///   Minimum length of time, in seconds, that BITS waits after encountering a transient error before trying to
        ///   transfer the file. The default retry delay is 600 seconds (10 minutes). The minimum retry delay that you
        ///   can specify is 60 seconds. If you specify a value less than 60 seconds, BITS changes the value to 60
        ///   seconds. If the value exceeds the no-progress-timeout value retrieved from the <see
        ///    cref="GetNoProgressTimeout" /> method, BITS will not retry the transfer and moves the job to the
        ///    BGJobStateError state. .
        /// </param>
        ...
        /// </summary>
        /// <param name="seconds">
        ///   Length of time, in seconds, that the service waits after encountering a transient error before trying to
        ///   transfer the file. .
        /// </param>
        ...
        /// </summary>
        /// <param name="seconds">
        ///   Length of time, in seconds, that BITS tries to transfer the file after the first transient error occurs.
        ///   The default retry period is 1,209,600 seconds (14 days). Set the retry period to 0 to prevent retries and
        ///   to force the job into the BGJobStateError state for all errors. If the retry period value exceeds the
        ...
        ///   proxy options, see the <see
        ///    cref="BGJobProxyUsage" /> enumeration.
        /// </param>
        /// <param name="proxyList">
        ///   A string that contains the proxies to use to transfer files. The list is space-delimited. For details on
        ///   specifying a proxy, see Remarks. This parameter must be <c>null</c> if the value of <see
        ///    cref="ProxyUsage" /> is BGJobProxyUsagePreConfig, BGJobProxyUsageNoProxy, or BGJobProxyUsageNoAutoDetect.
        ///    The length of the proxy list is limited to 4,000 characters, not including the <c>null</c> terminator. .
        /// </param>
        /// <param name="proxyBypassList">
        ///   A string that contains an optional list of host names, IP addresses, or both, that can bypass the proxy.
        ///   The list is space-delimited. For details on specifying a bypass proxy, see Remarks. This parameter must be
        ///   <c>null</c> if the value of <see
        ///    cref="ProxyUsage" /> is BGJobProxyUsagePreConfig, BGJobProxyUsageNoProxy, or BGJobProxyUsageNoAutoDetect.
        ...
        /// </param>
        /// <param name="proxyList">
        ///   A string that contains one or more proxies to use to transfer files. The list is space-delimited. For
        ///   details on the format of the string, see the Listing Proxy Servers section of Enabling Internet
        ///   Functionality. Call the CoTaskMemFree function to free <paramref
        ///    name="proxyList" /> when done.
        /// </param>
        /// <param name="proxyBypassList">
        ///   A string that contains an optional list of host names or IP addresses, or both, that were not routed
        ///   through the proxy. The list is space-delimited. For details on the format of the string, see the Listing
        ///   the Proxy Bypass section of Enabling Internet Functionality. Call the CoTaskMemFree function to free
        ///   <paramref
        ...
        /// </summary>
        /// <param name="program">
        ///   A string that contains the program to execute. The program parameter is limited to MAX_PATH characters,
        ///   not including the <c>null</c> terminator. You should specify a full path to the program; the method will
        ///   not use the search path to locate the program. To remove command line notification, set program and
        ///   parameters to <c>null</c>. The method fails if program is <c>null</c> and parameters is non-<c>null</c>. .
        /// </param>
        /// <param name="parameters">
        ///   A string that contains the parameters of the program in program. The first parameter must be the program
        ///   in program (use quotes if the path uses long file names). The parameters parameter is limited to 4,000
        ///   characters, not including the <c>null</c> terminator. This parameter can be <c>null</c>.
        /// </param>
        ...
        /// </summary>
        /// <param name="program">
        ///   A string that contains the program to execute when the job enters the error or transferred state. Call the
        ///   CoTaskMemFree function to free program when done. .
        /// </param>
        /// <param name="parameters">
        ///   A string that contains the arguments of the program in program. Call the CoTaskMemFree function to free
        ...
        /// </summary>
        /// <param name="progress">
        ///   Contains information that you use to calculate the percentage of the reply file transfer that is complete.
        ...
        ///   A string that contains the full path to the reply file. BITS generates the file name if
        ///   ReplyFileNamePathSpec is <c>null</c> or an empty string. You cannot use wild cards in the path or file
        ///   name, and directories in the path must exist. The path is limited to MAX_PATH, not including the
        ///   <c>null</c> terminator. The user must have permissions to write to the directory. BITS does not support
        ///   NTFS streams. Instead of using network drives, which are session specific, use UNC paths (for example,
        ...
        ///   authentication. For details, see the <see
        ///    cref="BGAuthCredentials" /> structure. If the job currently contains credentials with the same target and
        ///    scheme pair, the existing credentials are replaced with the new credentials. The credentials persist for
        ///    the life of the job. To remove the credentials from the job, call the IBackgroundCopyJob2::<see
