...
namespace SharpBits.Base
{
    ...
    /// <summary>
    ///   Use the IBackgroundCopyManager interface to create transfer jobs,retrieve an enumerator object that
    ...
    internal interface IBackgroundCopyManager
    {
        ...
        ///   A string that contains a display name for the job. Typically, the display name is used to identify the job in a user interface. Note that more than one job may have the same display name. Must not be <c>null</c>. The name is limited to 256 characters, not including the <c>null</c> terminator.
        /// </param>
        /// <param name="type">
        ///   Type of transfer job, such as BGJob_TYPE_DOWNLOAD. For a list of transfer types, see the BGJob_TYPE enumeration.
        /// </param>
        /// <param name="jobId">
        ///   Uniquely identifies your job in the queue. Use this identifier when you call the <see cref="IBackgroundCopyManager" />::<see
        ///   cref="GetJob" /> method to get a job from the queue.
        /// </param>
        /// <param name="job">
        ///   An <see cref="IBackgroundCopyJob" /> interface pointer that you use to modify the job's properties and specify the files to be transferred. To activate the job in the queue, call the <see
        ///    cref="IBackgroundCopyJob" />::Resume method. Release <paramref name = "job" /> when done.
        /// </param>
        void CreateJob(
        ...
        /// </param>
        /// <param name="enum">
        ///   An <see cref="IEnumBackgroundCopyJobs" /> interface pointer that you use to enumerate the jobs in the transfer queue. The contents of the enumerator depend on the value of <paramref
        ///    name = "flags" />. Release p@enumJobs when done.
        /// </param>
        ...
        ///   A string that contains a description of the error. Call the CoTaskMemFree function to free <paramref
