// ***********************************************************************
// <copyright file="IBackgroundCopyJob4.cs" project="SharpBits.Base" assembly="SharpBits.Base" solution="SevenUpdate" company="Xidar Solutions">
...
namespace SharpBits.Base
{
    ...
    internal interface IBackgroundCopyJob4
    {
        ...
        /// <param name="fileSet">Array of <c>BGFileInfo</c> structures that identify the local and remote file names of the files to transfer.</param>
        ...
        /// specifying the local name, see the LocalName member and Remarks section of the <see cref="BGFileInfo" />
        /// structure.</param> <param name="localName">A string that contains the name of the file on the server. For
        /// information on specifying the remote name, see the RemoteName member and Remarks section of the
        ...
        /// <summary>
        ///   Retrieves job-related progress information,such as the number of bytes and files transferred to the
        ///   client.
        /// </summary>
        ...
        /// transferring data, or suspended. For a list of job states, see the <see cref="BGJobState" /> enumeration
        /// type.</param>
        ...
        /// <param name="val">A string that contains the string version of the SID that identifies the job's owner. Call the CoTaskMemFree function to free ppOwner when done.</param>
        ...
        /// <param name="val">A string that identifies the job. Must not be <c>null</c>. The length of the string is limited to 256 characters, not including the <c>null</c> terminator. .</param>
        ...
        /// <param name="val">A string that contains the display name that identifies the job. More than one job can have the same display name. Call the CoTaskMemFree function to free displayName when done.</param>
        void GetDisplayName([MarshalAs(UnmanagedType.LPWStr)] out string val);

        /// <summary>Specifies a description of the job.</summary>
        /// <param name="val">A string that provides additional information about the job. The length of the string is limited to 1,024 characters, not including the <c>null</c> terminator.</param>
        void SetDescription([MarshalAs(UnmanagedType.LPWStr)] string val);

        /// <summary>Retrieves the description of the job.</summary>
        /// <param name="val">A string that contains a short description of the job. Call the CoTaskMemFree function to free ppDescription when done. .</param>
        ...
        /// <param name="val">Specifies the priority level of your job relative to other jobs in the transfer queue. The
        /// default is BGJobPriorityNormal. For a list of priority levels, see the <see cref="BGJobPriority" />
        /// enumeration. .</param>
        ...
        /// <summary>
        ///   Specifies a pointer to your implementation of the <c>IBackgroundCopyCallback</c> interface (callbacks).
        ///   The interface receives notification based on the event notification flags you set.
        /// </summary>
        /// <param name="val">An <c>IBackgroundCopyCallback</c> interface pointer. To remove the current callback interface pointer, set this parameter to <c>null</c>.</param>
        ...
        /// <param name="val">Interface pointer to your implementation of the <c>IBackgroundCopyCallback</c> interface. When done, release ppNotifyInterface.</param>
        void GetNotifyInterface([MarshalAs(UnmanagedType.IUnknown)] out object val);

        /// <summary>
        ///   Specifies the minimum length of time that BITS waits after encountering a transient error condition before
        ...
        /// <summary>
        ///   Retrieves the minimum length of time that BITS waits after encountering a transient error condition before
        ///   trying to transfer the file.
        /// </summary>
        ...
        /// <summary>
        ///   Specifies the length of time that BITS continues to try to transfer the file after encountering a
        ...
        /// <param name="proxyUsage">Specifies whether to use the user's proxy settings, not to use a proxy, or to use
        /// application-specified proxy settings. The default is to use the user's proxy settings,
        /// BGJobProxyUsagePreConfig. For a list of proxy options, see the <see cref="BGJobProxyUsage" />
        /// enumeration.</param> <param name="proxyList">A string that contains the proxies to use to transfer files.
        /// The list is space-delimited. For details on specifying a proxy, see Remarks. This parameter must be
        /// <c>null</c> if the value of <see cref="ProxyUsage" /> is BGJobProxyUsagePreConfig, BGJobProxyUsageNoProxy,
        /// or BGJobProxyUsageNoAutoDetect. The length of the proxy list is limited to 4,000 characters, not including
        /// the <c>null</c> terminator. .</param> <param name="proxyBypassList">A string that contains an optional list
        /// of host names, IP addresses, or both, that can bypass the proxy. The list is space-delimited. For details on
        /// specifying a bypass proxy, see Remarks. This parameter must be <c>null</c> if the value of <see
        ...
        /// proxy options, see the <see cref="BGJobProxyUsage" /> enumeration. .</param> <param name="proxyList">A
        /// string that contains one or more proxies to use to transfer files. The list is space-delimited. For details
        /// on the format of the string, see the Listing Proxy Servers section of Enabling Internet Functionality. Call
        /// the CoTaskMemFree function to free <paramref name="proxyList" /> when done.</param> <param
        /// name="proxyBypassList">A string that contains an optional list of host names or IP addresses, or both, that
        ...
        /// <summary>
        ///   Use the SetNotifyCmdLine method to specify a program to execute if the job enters the BGJobStateError or
        ///   BGJobStateTransferred state. BITS executes the program in the context of the user.
        /// </summary>
        ...
        void GetNotifyCmdLine([MarshalAs(UnmanagedType.LPWStr)] out string program, 
        ...
        ///   from an upload-reply job.
        /// </summary>
        /// <param name="progress">Contains information that you use to calculate the percentage of the reply file
        /// transfer that is complete. For more information, see <see cref="BGJobReplyProgress" />.</param>
        void GetReplyProgress([Out] out BGJobReplyProgress progress);

        /// <summary>
        ///   Use the GetReplyData method to retrieve an in-memory copy of the reply data from the server application.
        ///   Only call this method if the job's type is BGJobTypeUploadReply and its state is BGJobStateTransferred.
        /// </summary>
        /// <param name="buffer">Buffer to contain the reply data. The method sets buffer to <c>null</c> if the server application did not return a reply. Call the CoTaskMemFree function to free buffer when done.</param>
        /// <param name="length">Size, in bytes, of the reply data in buffer.</param>
        void GetReplyData(IntPtr buffer, out ulong length);

        /// <summary>
        ///   Use the SetReplyFileName method to specify the name of the file to contain the reply data from the server
        ///   application. Only call this method if the job's type is BGJobTypeUploadReply.
        /// </summary>
        /// <param name="replyFileName">A string that contains the full path to the reply file. BITS generates the file name if ReplyFileNamePathSpec is <c>null</c> or an empty string. You cannot use wild cards in the path or file name, and directories in the path must exist. The path is limited to MAX_PATH, not including the <c>null</c> terminator. The user must have permissions to write to the directory. BITS does not support NTFS streams. Instead of using network drives, which are session specific, use UNC paths (for example, \server\share\path\file). Do not include the \? prefix in the path. .</param>
        void SetReplyFileName([MarshalAs(UnmanagedType.LPWStr)] string replyFileName);

        /// <summary>
        ///   Use the GetReplyFileName method to retrieve the name of the file that contains the reply data from the
        ///   server application. Only call this method if the job type is BGJobTypeUploadReply.
        /// </summary>
        /// <param name="replyFileName">A string that contains the full path to the reply file. Call the CoTaskMemFree
        /// function to free <paramref name="replyFileName" /> when done. .</param>
        void GetReplyFileName([MarshalAs(UnmanagedType.LPWStr)] out string replyFileName);

        /// <summary>
        ///   Use the SetCredentials method to specify the credentials to use for a proxy or remote server user
        ///   authentication request.
        /// </summary>
        /// <param name="credentials">Identifies the target (proxy or server), authentication scheme, and the user's
        /// credentials to use for user authentication. For details, see the <see cref="BGAuthCredentials" /> structure.
        /// If the job currently contains credentials with the same target and scheme pair, the existing credentials are
        /// replaced with the new credentials. The credentials persist for the life of the job. To remove the
        /// credentials from the job, call the IBackgroundCopyJob2::<see cref="RemoveCredentials" /> method. .</param>
        void SetCredentials([In] ref BGAuthCredentials credentials);

        /// <summary>
        ///   Use the RemoveCredentials method to remove credentials from use. The credentials must match an existing
        ///   target and scheme pair that you specified using the IBackgroundCopyJob2::<c>SetCredentials</c> method.
        ///   There is no method to retrieve the credentials you have set.
        /// </summary>
        /// <param name="target">Identifies whether to use the credentials for proxy or server authentication.</param>
        /// <param name="scheme">Identifies the authentication scheme to use (basic or one of several challenge-response
        /// schemes). For details, see the <see cref="BGAuthScheme" /> enumeration. .</param>
        ...
        void AddFileWithRanges([MarshalAs(UnmanagedType.LPWStr)] string remoteUrl, 
                               [MarshalAs(UnmanagedType.LPWStr)] string localName, 
                               uint rangeCount, 
        ...
        /// <param name="elevated">If set to <c>True</c> [p elevated].</param>
