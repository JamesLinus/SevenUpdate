...
namespace Microsoft.Windows.Dialogs
{
    ...
    internal class NativeTaskDialog : IDisposable
    {
        ...
        internal NativeTaskDialog(NativeTaskDialogSettings settings, TaskDialog outerDialog)
        {
            ...
            nativeDialogConfig.pfCallback = new TaskDialogNativeMethods.PFTASKDIALOGCALLBACK(DialogProc);
        ...
        public DialogShowState ShowState { get { return showState; } }

        internal int SelectedButtonID { get { return selectedButtonID; } }

        [SuppressMessage("Microsoft.Performance", "CA1811:AvoidUncalledPrivateCode")]
        internal int SelectedRadioButtonID { get { return selectedRadioButtonID; } }

        internal bool CheckBoxChecked { get { return checkBoxChecked; } }
        ...
        private int HandleButtonClick(int id)
        {
            ...
            return id <= 9 ? outerDialog.RaiseClosingEvent(id) : 1;
        }
        ...
        private void UpdateTextCore(string s, TaskDialogNativeMethods.TASKDIALOG_ELEMENTS element)
        {
            ...
            SendMessageHelper(TaskDialogNativeMethods.TASKDIALOG_MESSAGES.TDM_SET_ELEMENT_TEXT, (int) element, (long) MakeNewString(s, element));
        }
        ...
        {
            AssertCurrentlyShowing();
            SendMessageHelper(TaskDialogNativeMethods.TASKDIALOG_MESSAGES.TDM_UPDATE_ICON, (int) element, (long) icon);
        }

        internal void UpdateCheckBoxChecked(bool cbc)
        {
            AssertCurrentlyShowing();
            SendMessageHelper(TaskDialogNativeMethods.TASKDIALOG_MESSAGES.TDM_CLICK_VERIFICATION, (cbc ? 1 : 0), 1);
        }

        internal void UpdateElevationIcon(int buttonId, bool showIcon)
        {
            AssertCurrentlyShowing();
            SendMessageHelper(TaskDialogNativeMethods.TASKDIALOG_MESSAGES.TDM_SET_BUTTON_ELEVATION_REQUIRED_STATE, buttonId, Convert.ToInt32(showIcon));
        }

        internal void UpdateButtonEnabled(int buttonID, bool enabled)
        {
            AssertCurrentlyShowing();
            SendMessageHelper(TaskDialogNativeMethods.TASKDIALOG_MESSAGES.TDM_ENABLE_BUTTON, buttonID, enabled ? 1 : 0);
        }

        internal void UpdateRadioButtonEnabled(int buttonID, bool enabled)
        {
            AssertCurrentlyShowing();
            SendMessageHelper(TaskDialogNativeMethods.TASKDIALOG_MESSAGES.TDM_ENABLE_RADIO_BUTTON, buttonID, enabled ? 1 : 0);
        }
        ...
        private int SendMessageHelper(TaskDialogNativeMethods.TASKDIALOG_MESSAGES msg, int wParam, long lParam)
        {
            ...
            return (int) CoreNativeMethods.SendMessage(hWndDialog, (uint) msg, (IntPtr) wParam, new IntPtr(lParam));
        }
